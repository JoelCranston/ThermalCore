package cofh.thermal.core.client.gui.device;

import cofh.thermal.core.common.inventory.device.DeviceCollectorMenu;
import cofh.thermal.lib.client.gui.AugmentableTileScreen;
import net.minecraft.network.chat.Component;
import net.minecraft.resources.Identifier;
import net.minecraft.world.entity.player.Inventory;

import static cofh.core.util.helpers.GuiHelper.generatePanelInfo;
import static cofh.lib.util.constants.ModIds.ID_THERMAL;

public class DeviceCollectorScreen extends AugmentableTileScreen<DeviceCollectorMenu> {

    public static final String TEX_PATH = ID_THERMAL + ":textures/gui/container/collector.png";
    public static final Identifier TEXTURE = Identifier.parse(TEX_PATH);

    public DeviceCollectorScreen(DeviceCollectorMenu container, Inventory inv, Component titleIn) {

        super(container, inv, container.tile, titleIn);
        texture = TEXTURE;
        info = generatePanelInfo("info.thermal.device_collector");
    }

}

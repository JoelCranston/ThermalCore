...
ThermalCore depends on **CoFHCore** (`../CoFHCore`), the foundation library for the
whole "Thermal Series". **ThermalDynamics** and **ThermalExpansion** are siblings at the
same level. All four repos are worked in **one shared Claude Code session**, in
dependency order each hop — CoFHCore first, then this repo and the others.
...
**uncommitted** — waiting on CoFHCore to compile clean on 1.20.6 first, since this repo
depends on it and would just inherit its breakage. See
[docs/TODO.md](docs/TODO.md).

**Next step**: once `../CoFHCore` compiles clean on 1.20.6 (check its
`docs/TODO.md`/`docs/progress-log.md`), commit the version bump here and start this
repo's own 1.20.6 migration, checking `../CoFHCore/docs/api-notes-1.20.6.md` first for
API shapes already confirmed (ItemStack NBT→DataComponents, MobEffect→Holder,
networking→StreamCodec, and more — full list there).
